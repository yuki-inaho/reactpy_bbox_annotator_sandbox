...
### 手順 23: 完了の定義チェック
- [ ]  **操作**: docs/workdoc.md の「6. 完了の定義」のチェックリストを確認し、全項目が満たされているか検証。
- [ ]  **確認**: 全てのチェック項目が [ ] から [x] に変更可能。
- [ ]  **テスト**: E2Eテスト 7/7 PASSED、README/justfile 存在、セキュリティ対応完了を確認。
- [ ]  **エラー時対処**: 未完了項目があれば該当手順に戻る。

### 手順 24: 最終commit & push
- [ ]  **操作**: 全ての変更を git add し、詳細なコミットメッセージで commit、push する。
- [ ]  **確認**: git status が clean、リモートに push 完了。
- [ ]  **テスト**: `git log -1 --oneline` で最新コミットを確認。
- [ ]  **エラー時対処**: push 失敗時はネットワーク確認、403エラー時はブランチ名確認。
